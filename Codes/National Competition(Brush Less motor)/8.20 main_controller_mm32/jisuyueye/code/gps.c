...
/*********GPS相关变量*********/
//直道换点角度
float direc_change_dis = 45.0;
//弯道换点角度
float round_change_dis = 45.0;
//目标圈数
uint16 circle_time = 2;
//停车距离
float stop_distance = 20.0;
...
void GpsAngleError()
{
    ...
    //在操场，先用用gps角
    //第一个点用陀螺仪做
    if (passed_sections <= 0 && run_circle == 0 && 0) 
    {
        //利用GPS角度与陀螺仪角度
...
void GpsDotSelection()
{
    ...
    {
        if (Use_Data.distan_next_point_now < direc_change_dis || Use_Data.change_angle_delta > 30.0)
        {
            change_point = 1;
        }
    }
    else if (Use_Data.distan_next_point_now < round_change_dis)
    {
          change_point = 1;
    }
