...
//用于划定区域的微操角度
double area_base_angle1 = 0.0;  //从1到2
double area_base_angle2 = 0.0;  //从0到3
...
void TIM5_IRQHandler (void)
{
	...
	if (total_time % 2 == 0 && 0)
	{
		...
		Data_Gyro.angle_sum_once =  Data_Gyro.angle_sum_once  - Data_Gyro.gyro_z / 500;
                ...
                //Kalman_Filter( icm_acc_z , Data_Gyro.gyro_z);	
                
                //Data_Gyro.angle_sum_all = Data_Gyro.angle_sum_all + angle_dot / 500;
                
                //Yijielvbo(- Data_Gyro.gyro_z / 500 , - Data_Gyro.gyro_z);
               // Data_Gyro.angle_sum_all = First_Order_Filter_Calc(icm_acc_x, icm_acc_y, Data_Gyro.gyro_z, Data_Gyro.angle_sum_all);
                //Data_Gyro.angle_sum_area =  Yijielvbo( - Data_Gyro.gyro_z / 500 , - Data_Gyro.gyro_z ,Data_Gyro.angle_sum_area);
                                     
                //计算水平误差(直道区域)
                //米为单位
                if (area_tag == 0 && (total_time % 10 == 0) )
                {
                    //0.01s一次调节
                    horizontal_error = horizontal_error + sin (Data_Gyro.angle_sum_area) *  Save_Data.f_ground_speed * 0.01 / 3.6;
                }
                else //弯道不做考虑
                {
	...
	//舵机控制
	if (total_time % 20 == 0 && (gps_mode == 1 || gps_mode == 2) && (g_car_speed > 100.0 || Save_Data.f_ground_speed > 3.15) )
	{
		//如果取4的话震荡过大
		gps_servo_control();
	...
	if (total_time % 400 == 0)
    {
        ...
        if (total_time % 10 == 0)
        {
              if (Save_Data.f_ground_speed > 1.0)
              {
                  distance_from_lastpoint =  distance_from_lastpoint + (Save_Data.f_ground_speed / 3.6 )* 0.01;
              }
        }
	...
	else if (strcmp(CarCtrlOrder, "t") == 0)
	{
                //返回应答
                send_order = 1;
		strcpy(CarCtrlOrder, "");
	...
	if (newgpsgot)
	{
		...
		if (Save_Data.isUsefull == true && gps_mode == 1 && start_delay_time <= 0)
		{
                        ...
                        /***************起步区*****************/      
			//防止微扰动
			if ( passed_sections == 0 && ((g_car_speed < 100.0) ||  (Save_Data.f_ground_speed < 0.8)) )
			{
				pwm_set_duty(SERVO_PIN, pwm_mid_angle);
                                //现在点与真实起点的误差
                                error_lenth_s_p = GPSDistance(Save_Data.f_lati, Save_Data.f_longi,Points_Use[0].lat_keypoint, Points_Use[0].lon_keypoint);
                                error_direction = GPSBearingAngle(Save_Data.f_lati, Save_Data.f_longi,Points_Use[0].lat_keypoint, Points_Use[0].lon_keypoint);
			}
			...
			if (passed_sections <= run_sections && dot_read == false && analysis_finished == true)
			{
				...
				if (Points_Use[passed_sections + 1].type_point != Points_Use[passed_sections].type_point)
				{
					Data_Gyro.angle_sum_area = 0.0;
                                ...
                                //数据问题，紧急停车                          
                                if (Use_Data.lat_next < 20.0 || isnan(Use_Data.lat_next))
                                {
                                    //速度归零
                                    g_speed_set = 0.0;
                                    setspeed_to_pwm();
				...
				dot_read = true;
                              ...
                              //速度上不去，航向角求出来也是错的
                              //测试用最低速度设置为2m/s
                              if (Save_Data.f_ground_speed > 3.15 || g_car_speed > 100.0)
                              {
                ...
                else if (gps_mode == 55)
                {
                      collection_by_hand();
                      //如果采集了四个点，就进行赋值
                      if (finished_gpspoints == 4)
                      {
                        kps_hust_est_plygrnd[0].kps_latitude = Points_Stored[0].lat_keypoint;
                        kps_hust_est_plygrnd[0].kps_longitude = Points_Stored[0].lon_keypoint;
                        kps_hust_est_plygrnd[0].kps_type = 0;
                        //点1
                        kps_hust_est_plygrnd[1].kps_latitude = Points_Stored[1].lat_keypoint;
                        kps_hust_est_plygrnd[1].kps_longitude = Points_Stored[1].lon_keypoint;
                        kps_hust_est_plygrnd[1].kps_type = 1;

                        //点2
                        kps_hust_est_plygrnd[2].kps_latitude = Points_Stored[2].lat_keypoint;
                        kps_hust_est_plygrnd[2].kps_longitude = Points_Stored[2].lon_keypoint;
                        kps_hust_est_plygrnd[2].kps_type = 2;

                        //点3
                        kps_hust_est_plygrnd[3].kps_latitude = Points_Stored[3].lat_keypoint;
                        kps_hust_est_plygrnd[3].kps_longitude = Points_Stored[3].lon_keypoint;;
