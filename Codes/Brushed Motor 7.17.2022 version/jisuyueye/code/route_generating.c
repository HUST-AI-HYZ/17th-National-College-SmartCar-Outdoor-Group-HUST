...
//东操场
FourPoints kps_hust_est_plygrnd[4];
RPoints rps_hust_est_plygrnd[150];
...
{
   //点2
	kps_hust_est_plygrnd[0].kps_latitude = 30.483757;
	kps_hust_est_plygrnd[0].kps_longitude = 114.304831;
	kps_hust_est_plygrnd[0].kps_type = 0;
        
        //点3
	kps_hust_est_plygrnd[1].kps_latitude = 30.484545;
	kps_hust_est_plygrnd[1].kps_longitude = 114.304738;
	kps_hust_est_plygrnd[1].kps_type = 1;
        
  
    kps_hust_est_plygrnd[2].kps_latitude = 30.484449;
	kps_hust_est_plygrnd[2].kps_longitude = 114.303866;
	kps_hust_est_plygrnd[2].kps_type = 2;
	//点1
	kps_hust_est_plygrnd[3].kps_latitude = 30.483688;
	kps_hust_est_plygrnd[3].kps_longitude = 114.303975;
	kps_hust_est_plygrnd[3].kps_type = 3;
...
void generating_route()
{
	...
	//处理第一段直道
	rps_hust_est_plygrnd[0].rps_latitude = kps_hust_est_plygrnd[0].kps_latitude;
	rps_hust_est_plygrnd[0].rps_longitude = kps_hust_est_plygrnd[0].kps_longitude;
	rps_hust_est_plygrnd[0].rps_type = 0;
	generate_direct_road(distance_each_point_direct, kps_hust_est_plygrnd[0], kps_hust_est_plygrnd[1] ,&delt_dots);
	generate_round_road(distance_each_point_round, kps_hust_est_plygrnd[1], kps_hust_est_plygrnd[2], &delt_dots);
	generate_direct_road(distance_each_point_direct, kps_hust_est_plygrnd[2], kps_hust_est_plygrnd[3], &delt_dots);
	generate_round_road(distance_each_point_round, kps_hust_est_plygrnd[3], kps_hust_est_plygrnd[0], &delt_dots);

	//校验组
	for (size_t i = 0; i < delt_dots + 1; i++)
	{
		exam_distance[i] = GPSDistance(rps_hust_est_plygrnd[i].rps_latitude, rps_hust_est_plygrnd[i].rps_longitude, rps_hust_est_plygrnd[i + 1].rps_latitude, rps_hust_est_plygrnd[i + 1].rps_longitude);
		exam_direction[i] = GPSBearingAngle(rps_hust_est_plygrnd[i].rps_latitude, rps_hust_est_plygrnd[i].rps_longitude, rps_hust_est_plygrnd[i + 1].rps_latitude, rps_hust_est_plygrnd[i + 1].rps_longitude);
	}
...
int generate_round_road(float step, FourPoints strt, FourPoints end ,int *delt_dots)
{
	...
	while (i < times + 1)
	{
		...
		//第一次分析
		rps_hust_est_plygrnd[*delt_dots + i].rps_latitude = mid_lati + direction_vector_lati * dis_ratio;
		rps_hust_est_plygrnd[*delt_dots + i].rps_longitude = mid_longi + direction_vector_longi * dis_ratio;
		rps_hust_est_plygrnd[*delt_dots + i].rps_type = 1;

		
		gene_dis1[i] = GPSDistance(rps_hust_est_plygrnd[*delt_dots + i].rps_latitude, rps_hust_est_plygrnd[*delt_dots + i].rps_longitude, rps_hust_est_plygrnd[*delt_dots + i - 1].rps_latitude, rps_hust_est_plygrnd[*delt_dots + i - 1].rps_longitude);
		...
		if (gene_dis1[i] < step * 0.90 || gene_dis1[i] > step * 1.10)
		{
			...
			//再来一次
			rps_hust_est_plygrnd[*delt_dots + i].rps_latitude = rps_hust_est_plygrnd[*delt_dots + i - 1].rps_latitude + (rps_hust_est_plygrnd[*delt_dots + i].rps_latitude - rps_hust_est_plygrnd[*delt_dots + i - 1].rps_latitude) * error_ratio;
			rps_hust_est_plygrnd[*delt_dots + i].rps_longitude = rps_hust_est_plygrnd[*delt_dots + i - 1].rps_longitude + (rps_hust_est_plygrnd[*delt_dots + i].rps_longitude - rps_hust_est_plygrnd[*delt_dots + i - 1].rps_longitude) * error_ratio;
			rps_hust_est_plygrnd[*delt_dots + i].rps_type = 1;
		}
		else
		{
		...
		di_angle[i] = GPSBearingAngle(mid_lati, mid_longi, rps_hust_est_plygrnd[*delt_dots + i].rps_latitude, rps_hust_est_plygrnd[*delt_dots + i].rps_longitude);
		angle_sum = delta_heading_angle(basic_angle, di_angle[i]);

		gene_dis2[i] = GPSDistance(rps_hust_est_plygrnd[*delt_dots + i].rps_latitude, rps_hust_est_plygrnd[*delt_dots + i].rps_longitude, rps_hust_est_plygrnd[*delt_dots + i - 1].rps_latitude, rps_hust_est_plygrnd[*delt_dots + i - 1].rps_longitude);
		gene_dis3[i] = GPSDistance(rps_hust_est_plygrnd[*delt_dots + i].rps_latitude, rps_hust_est_plygrnd[*delt_dots + i].rps_longitude, mid_lati, mid_longi);
		dis_ratio = 0.001;
		
		if (delta_heading_angle(basic_angle, di_angle[i]) > 75)
		{	
			passbyhalf = 1;
		}

		if (delta_heading_angle(basic_angle, di_angle[i]) > 180 - 0.5 * delta_sita)
		{       
                        out_way = false;
			break;
		}
	...
	*delt_dots = *delt_dots + i;
	double dis_l = GPSDistance(end.kps_latitude, end.kps_longitude, rps_hust_est_plygrnd[*delt_dots].rps_latitude, rps_hust_est_plygrnd[*delt_dots].rps_longitude);

	if (dis_l < distance_each_point_round)
	{
		rps_hust_est_plygrnd[*delt_dots].rps_latitude = end.kps_latitude;
		rps_hust_est_plygrnd[*delt_dots].rps_longitude = end.kps_longitude;
		rps_hust_est_plygrnd[*delt_dots].rps_type = 1;
	}
	else
	{
		rps_hust_est_plygrnd[*delt_dots].rps_latitude = (end.kps_latitude + rps_hust_est_plygrnd[*delt_dots - 1].rps_latitude)*0.5;
		rps_hust_est_plygrnd[*delt_dots].rps_longitude = (end.kps_longitude + rps_hust_est_plygrnd[*delt_dots - 1].rps_longitude)*0.5;
		rps_hust_est_plygrnd[*delt_dots].rps_type = 1;
...
{	
	...
	while (i < stepnumber)
	{
		rps_hust_est_plygrnd[*delt_dots + i].rps_latitude = rps_hust_est_plygrnd[*delt_dots + i - 1].rps_latitude + lati_step;
		rps_hust_est_plygrnd[*delt_dots + i].rps_longitude = rps_hust_est_plygrnd[*delt_dots + i - 1].rps_longitude + longi_step;
		rps_hust_est_plygrnd[*delt_dots + i].rps_type = 0;

		gene_dis1[i] = GPSDistance(rps_hust_est_plygrnd[*delt_dots + i].rps_latitude, rps_hust_est_plygrnd[*delt_dots + i].rps_longitude, end.kps_latitude, end.kps_longitude);
		gene_dis2[i] = GPSDistance(rps_hust_est_plygrnd[*delt_dots + i].rps_latitude, rps_hust_est_plygrnd[*delt_dots + i].rps_longitude, rps_hust_est_plygrnd[*delt_dots + i - 1].rps_latitude, rps_hust_est_plygrnd[*delt_dots + i - 1].rps_longitude);
		i++;
	}
	//最后一个直接连接上
	rps_hust_est_plygrnd[*delt_dots + i].rps_latitude = end.kps_latitude;
	rps_hust_est_plygrnd[*delt_dots + i].rps_longitude = end.kps_longitude;
	rps_hust_est_plygrnd[*delt_dots + i].rps_type = 0;
...
int write_in(int delt_dots, double* dis, double* direction)
{
	...
	for (int i = 0; i < delt_dots; i++)
	{
      int16 hund = 0, ten = 0;
      //double部分
		Points_Stored[i].lon_keypoint = rps_hust_est_plygrnd[i].rps_longitude;
      Points_Stored[i].lat_keypoint = rps_hust_est_plygrnd[i].rps_latitude;

      //整数小数分离
      ten = (int)(rps_hust_est_plygrnd[i].rps_latitude /10);
      Points_Stored[i].lat_kypnt_z =  ten * 10 + (int)(rps_hust_est_plygrnd[i].rps_latitude /1 - (ten * 10 ) ) ;
      Points_Stored[i].lat_kypnt_f = rps_hust_est_plygrnd[i].rps_latitude - Points_Stored[i].lat_kypnt_z;
      //整数小数分离
      hund = (int)(rps_hust_est_plygrnd[i].rps_longitude /100);
      ten = (int)(rps_hust_est_plygrnd[i].rps_longitude /10) - hund * 10;
      Points_Stored[i].lon_kypnt_z = hund*100 + ten*10 + (int)(rps_hust_est_plygrnd[i].rps_longitude - (hund*100 + ten*10));
      Points_Stored[i].lon_kypnt_f = rps_hust_est_plygrnd[i].rps_longitude - Points_Stored[i].lon_kypnt_z;
      //点类型
      Points_Stored[i].type_point = rps_hust_est_plygrnd[i].rps_type;
	}
...
#define PNTNUM 4
    ...
    if( fabs(delta_heading_angle(angle1,angle2) < 100) )
    {
        ...
        Points_Use[0].type_point = Points_Stored[min_num].type_point;
...
void round_in_out()
{
	...
	for (int i = 0;i < finished_gpspoints;i++)
	{
		...
		if (last_type_tag != Points_Stored[i].type_point)
		{
			...
			{
				Points_Stored[i].type_point = ROUND_IN_AREA;
                                Points_Stored[i - 1].type_point = ROUND_IN_AREA;
                                Points_Stored[i - 2].type_point = ROUND_IN_AREA;
				last_type_tag = 1;
			}
			else
			{
				Points_Stored[i].type_point = ROUND_OUT_AREA;
                                Points_Stored[i + 1].type_point = ROUND_OUT_AREA;
				last_type_tag = 0;
                                i = i + 1;
			}
		}
	...
	Points_Stored[0].type_point = ROUND_OUT_AREA;
        Points_Stored[finished_gpspoints - 1].type_point = ROUND_OUT_AREA;
     ...
     static bool collect_flag = false;
                        ...
                        if (button_mod % 2 == 1 && collect_flag == false)
                        {
                            if (gpio_get(SWITCH_4) == 0) 
                            {
                                ...
                                if(Save_Data.lati_Z == 30 && Save_Data.longi_Z == 114)
                                {
                              ...
                              if(Save_Data.lati_Z == 30 && Save_Data.longi_Z == 114)
                                {
                                    ...
                                    finished_gpspoints++;
                                   collect_flag = false;
                                    tft180_show_string(0, 16*7, "SAVEPNTS");
                                    FlashWhitePoints();
                                }
                            }
